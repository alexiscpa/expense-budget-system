...
版本：v1.2（v1.1 對 12331／12401 的裁決有誤，已修正；見版本紀錄）
...
| **合計** | **45** | （PARENT_ONLY 17、MERGED 1、INACTIVE 5、NEEDS_CONFIRMATION 0，共 68；v1.2 更新，見版本紀錄） |

---

## 3　PARENT_ONLY／INACTIVE／MERGED／NEEDS_CONFIRMATION 清單

### 3.1　PARENT_ONLY（17 筆，純上層彙總，不直接輸入）

前 12 筆（不含 16104）為 v1.0 原始盤點結果，`16104` 於 v1.2 補上 exclusionReason 註記（disposition 自 v1.0 起即為 PARENT_ONLY，未變更、不重複計數）；後 4 筆（標示「v1.2 人工裁決」）原為 §3.4 的 NEEDS_CONFIRMATION，經人工明確確認為上層彙總單位，於 v1.1／v1.2 改列於此——原始調查證據（出現位置、明細頁、公式）全數保留，未刪除任何部門紀錄。`12331` 已於 v1.2 改列 §3.3 INACTIVE（原始名稱標示「無」，非上層彙總單位），不再列於本節。
...
### 3.3　INACTIVE（5 筆，已停用或標示「無」）

前 4 筆為 v1.0 原始盤點結果；第 5 筆（`12331`，標示「v1.2 人工裁決」）原於 v1.1 誤列為 §3.1 PARENT_ONLY，人工於 v1.2 更正——原始名稱本身已標示「無」，正確分類是 INACTIVE 而非上層彙總單位；原始調查證據全數保留，未刪除任何部門紀錄。
