...
| 33 | 12331 | 海外行銷部(無) | S | 是 | NEEDS_CONFIRMATION | 12301 | — |
| 34 | 12401 | 營業企劃處 | S | 否 | NEEDS_CONFIRMATION | — | — |
| 35 | 12411 | 成品企劃部 | S | 否 | BUDGET_OWNER | 12401 | — |
| 36 | 12421 | 客戶服務部 | S | 否 | BUDGET_OWNER | 12401 | — |
| 37 | 12501 | 教研營業處 | S | 否 | NEEDS_CONFIRMATION | — | — |
| 38 | 13001 | 第二營業本部 | S | 否 | NEEDS_CONFIRMATION | — | — |
...
| **合計** | **45** | （PARENT_ONLY 13、MERGED 1、INACTIVE 4、NEEDS_CONFIRMATION 5，共 68） |

---

## 3　PARENT_ONLY／INACTIVE／MERGED／NEEDS_CONFIRMATION 清單

### 3.1　PARENT_ONLY（13 筆，純上層彙總，不直接輸入）
...
| 16104 | 台灣廠 | B.營管生產與編制!row64 = SUM(生產+生技+資材+採購)，不含品保 |
...
### 3.4　NEEDS_CONFIRMATION（5 筆，來源不足，需人工確認）

| 代碼 | 名稱 | 為何無法自行判定 |
|---|---|---|
| 12331 | 海外行銷部(無) | ERP部門標「無」且明細頁名含「(x)」停用標記，但該頁仍被 2025 彙總公式實際加總（B表row5「海外行銷企劃」＝1,290,000），與「無」矛盾 |
| 12401 | 營業企劃處 | 全檔零出現（含歷史帳），但子代碼 12411/12421 是活躍 BUDGET_OWNER，與同類型的 12201/12301 不同（後兩者都有明確 SUM 彙總列，12401 完全沒有） |
| 12501 | 教研營業處 | 無 2025 彙總或明細頁，但歷史帳有 3 筆紀錄，非「無」標記 |
| 13001 | 第二營業本部 | 全檔零出現（含歷史帳），是除 A1004 外唯一完全零蹤跡的代碼，但未被標「無」 |
| 14001 | 第三營業本部 | 無 2025 彙總或明細頁，歷史帳有 8 筆紀錄 |
...
### 風險 6（低）：12401「營業企劃處」完全查無蹤跡

- 與同構型的 12201／12301 都有一個明確的 SUM 彙總列不同，12401 連一個彙總列都沒有——若 Stage 2B 之後需要「四大體系 → 二級處 → 三級部」的完整三層階層，12401 這一層目前無法從本檔案重建。
...
**前提：等待人工確認完整部門名冊（含 §3.4 的 5 筆 NEEDS_CONFIRMATION、§4 的 2 組合併案例決定方案、§5 風險 3 的 M/S 前綴方向決定）後才可開始，本階段不自行進入。**
...
4. 針對 §3.4 NEEDS_CONFIRMATION 清單，每一筆一旦被人工裁決，都應該有一則對應的 reconciliation manifest 更新 + 至少一則說明其歸類理由的測試/文件，避免下次改版時又要重新調查一次。
