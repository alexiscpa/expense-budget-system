  ...
  return (
    <div className="mb-8 rounded border border-amber-300 bg-amber-50 p-4">
      ...
      <p className="mb-1 text-xs text-amber-700">
        ...
        2026 年度測試預算。可重複執行，不會產生重複資料。科目編號採來源檔案 A 欄「序號」原值（例如序 3 對應「薪資支出」）。
      </p>
      ...
      {result && (
        <div className="mt-3 rounded bg-white p-3 text-xs text-slate-700">
          ...
          <p>
            明細科目筆數：{result.accountCount} 筆（科目編號 {result.accounts[0]?.code ?? "-"} ～{" "}
            {result.accounts[result.accounts.length - 1]?.code ?? "-"}）
          </p>
