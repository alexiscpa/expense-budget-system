...
### 本次新增：費用預算彙總表（版型預覽）— 階段一：純版面預覽

`/dashboard/reports/budget-summary-preview` — **僅版型預覽，非正式彙總功能**。目的是讓使用者在實際套用
真實版面前，先確認欄位、配色、捲動與操作方式；**本階段刻意不建立任何新 Department、不匯入任何歷史金額、
不異動 Prisma schema、不新增 migration、完全唯讀、不寫入資料庫**。

- **入口與存取控制**：Dashboard 新增「費用預算彙總表（版型預覽）」入口方塊（僅 Preview bypass 啟用時顯示）
  與明確標示「版型預覽：目前僅財務管理處為實際測試資料，其他部門尚未匯入。」頁面本身沿用既有
  `isAuthBypassEnabled()`（`lib/env.ts`）——與 DEMO 主檔初始化等既有 Preview-only 功能完全相同的
  fail-closed 閘門，Production 一律硬擋，非 Preview+`AUTH_DISABLED=true` 時呼叫 `notFound()`。已實測：
  停用 bypass 後直接連到此路徑會被既有 middleware 攔截導回 `/login`（與其他 `/dashboard/*` 路由行為一致）。
- **三個頁籤**：「單位別費用與編制」（研發／營業／管理／生產四大區塊，兩層表頭）、「管銷研科目彙總」
  （營業＋管理＋研發，序＋項目＋2026推估＋2027目標不含新員/新員/合計＋不含新員/含新員增減與成長率，明確
  排除生產）、「生產科目彙總」（欄位結構相同，因尚未匯入生產資料，數字全部顯示 `—`，狀態顯示
  「尚未匯入生產部門資料」，不建立假 BudgetVersion/BudgetLine）。科目彙總頁上方另有「全公司費用合計區」
  （管銷研費用合計／生產費用合計／全公司費用總計＋勾稽狀態訊息「全公司費用尚無法完成彙總：生產單位資料
  尚未匯入。」），兩個科目彙總頁籤共用同一份彙總表。
- **資料來源（2026-09-08 修正）**：財務管理處（17203）**只**讀取 fiscalYear=2027 的 `BudgetVersion`（唯讀
  查詢，從不建立或修改）——**不再**挑選「該部門最近一次異動」的任意版本，避免既有 2026 預算資料被誤標為
  2027 目標。該 2027 版本的 `priorYearOriginalBudget`（即 `Account.priorYearReferenceAmount`）才顯示為
  「2026推估」；尚無 fiscalYear=2027 版本時，單位別頁籤該部門狀態顯示「2027年度尚未編製」（與其他代表性
  部門的通用「未編製」區分），管銷研／生產頁籤顯示對應提示文字，所有欄位皆為 `—`。頁面版型示意數字皆
  明確標示「版型示意數字」字樣，不宣稱為正式 2027 預算。其他所有部門（研發一部、台北、GWK、董事長室、
  生產部…）僅為 `src/lib/reports/budgetSummaryPreviewData.ts` 中手打的代表性名稱字串，未編製一律顯示 `—`
  （絕不顯示為 `0`），狀態顯示「未編製」；每一區塊末端的「體系小計」只加總已有實際資料的部門，並標示
  「暫計，尚有未編製部門」。海外單位（GWK／GWH／GWSEA／GWS／GWA／GWI／TEXIO／大陸環測／蘇州廠／其他海外
  單位）全數歸入「營業單位」區塊，不獨立成區。
- **樣式**：區塊名稱紅字、年度群組表頭灰底藍字、一般表頭較深灰底、小計淡橘底、體系/分類/最終總計黃底、
  負數紅色括號（如 `(5,753,051)`）、正數千分位、成長率四捨五入至小數一位（無法計算顯示 `—`）。每張表格
  皆為獨立的「表頭垂直固定＋第一欄或序＋項目欄水平固定」捲動容器（`StickyReportTable`），欄寬刻意加寬到
  超出頁面內容寬度，確保三個測試寬度（1366／1440／1920px）下都需要、也都能正確水平捲動並驗證凍結欄位。
- 新增 `lib/reports/summaryFormat.ts`（純格式化函式：千分位／紅色括號負數／成長率一位小數／`—` 佔位，
  已 unit test）與 `lib/reports/budgetSummaryPreviewData.ts`（純資料：四大區塊代表性部門清單、海外單位
  歸類、生產頁籤佔位列）。

**驗證結果**：新增 `tests/budgetSummaryPreviewFormat.test.ts`（14 項，涵蓋金額/人數/成長率格式化、海外
單位全數歸入營業單位、生產單位與管銷研三區塊互斥、財務管理處歸入管理單位、四大區塊最少代表性部門齊全、
生產頁籤佔位列齊全）；`npm run lint`／`typecheck`／`test`（20 個檔案、**168 個測試全數通過**）／
`npm run build` 皆成功；**未新增 Prisma schema 變更、未新增 migration**（`git diff --stat -- prisma/`
確認為空）；未修改財務管理處預算輸入／送出／撤回／部門人數任一既有功能檔案；實際啟動 `next dev` 並以
Playwright 完成三個頁籤切換、財務管理處真實資料顯示（10人/22,176,065 → 20人/88,000，負數與成長率皆為
紅色括號）、未編製部門全數顯示 `—`（非 `0`）、海外單位全數出現在營業單位、管銷研排除生產／生產排除管銷研
科目名稱、水平捲動後序／項目欄正確固定於畫面左側且與捲動內容不重疊、1366/1440/1920px 三種寬度皆可正常
操作；資料庫列數比對（Account 62／Department 1／BudgetVersion 7／BudgetLine 434）確認本次操作全程無任何
寫入。
