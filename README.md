...
- **三個頁籤**：「單位別費用與編制」（研發／營業／管理／生產四大區塊，兩層表頭）、「管銷研科目彙總」
  （營業＋管理＋研發，序＋項目＋2025推估＋2026不含新員/新員/合計＋不含新員/含新員增減與成長率，明確
...
- **資料來源**：財務管理處（17203）使用系統既有的真實 `BudgetVersion`（挑選該部門 fiscalYear/versionNumber/
  updatedAt 最新的一筆，唯讀查詢，從不建立或修改）；其他所有部門（研發一部、台北、GWK、董事長室、生產部…）
  僅為 `src/lib/reports/budgetSummaryPreviewData.ts` 中手打的代表性名稱字串，未編製一律顯示 `—`（絕不
  顯示為 `0`），狀態顯示「未編製」；每一區塊末端的「體系小計」只加總已有實際資料的部門，並標示「暫計，
  尚有未編製部門」。海外單位（GWK／GWH／GWSEA／GWS／GWA／GWI／TEXIO／大陸環測／蘇州廠／其他海外單位）
  全數歸入「營業單位」區塊，不獨立成區。
